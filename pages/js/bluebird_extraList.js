    ...
    function 高级列表框(name,haveArrow,haveMsg,lazy,functionName){  
        ...
        this.插入项目 = function (index,img,title,msg,tag){
            var table = document.getElementById(this.名称);
            var li = document.createElement('li');
            var temp = '<div class="mui-media-body">'+title;
            if(this.显示信息==true){
            ...
            li.innerHTML = temp;
            var len = table.getElementsByTagName("li").length;
            var result = index;
            if(index<1){
                result = 0;
                ...
                result = index;
                table.insertBefore(li,table.childNodes[result+1]);
            }
        ...
        this.插入项目2 = function (index,img,title,msg,tag){
            var table = document.getElementById(this.名称);
            var li = document.createElement('li');
            var temp = '<div class="mui-media-body">'+title;
            if(this.显示信息==true){
        ...
        this.取项目标记 = function (index){
            ...
            var tag = li[index].getAttribute("tag");
            //tag = tag.replace(/(^\n*)|(\n*$)/g, ""); //去掉首尾的换行符
            return tag;
        }
        ...
        if(functionName!=null){
            mui('#'+this.名称).on('tap', 'li', function() {
                ...
                //tag = tag.replace(/(^\n*)|(\n*$)/g, ""); //去掉首尾的换行符
                functionName(Number(this.index),image,title,msg,tag);//表项被单击事件，返回项目索引、项目图片、项目标题、项目信息、项目标记
            });       	
        }
    }  
