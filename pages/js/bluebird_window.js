    (function (){  
        ...
        function 置组件宽度(name, value) {
            ...
            var div = document.querySelector("#" + name);
            if (!div) {
                console.error("组件未找到: " + name);
                return;
            }

            // 检查父节点是否存在，并处理逻辑
            if (div.parentNode && !div.parentNode.classList.contains("mui-content")) {
                if (
                    div.parentNode.getAttribute("tag") == "自由面板" || 
                    (div.parentNode.parentNode && div.parentNode.parentNode.getAttribute("tag") == "自由面板")
                ) {
                    // 自由面板上的组件
        ...
        function 置组件高度(name, value) {
            ...
            if (div.parentNode && !div.parentNode.classList.contains("mui-content")) {
                // 检查是否是自由面板组件
                if (div.parentNode.getAttribute("tag") == "自由面板" || 
                    (div.parentNode.parentNode && div.parentNode.parentNode.getAttribute("tag") == "自由面板")) {
                    // 自由面板上的组件
        ...
        function 置组件位置尺寸(name,left,top,width,height){
            var div = document.querySelector("#"+name);
            if(div.parentNode.className!="mui-content"){
                if(div.parentNode.getAttribute("tag")=="自由面板" || div.parentNode.parentNode.getAttribute("tag")=="自由面板"){
                    //自由面板上的组件
                    div.parentNode.style.left = left;
                    div.parentNode.style.top = top;
        ...
        function 置组件距离顶部(app,value,mold){
            if(mold == true){
        ...
        function 置组件距离顶部2(app,value,mold){
            if(mold == true){
                var div = document.getElementById(app).parentNode;
        ...
        function 隐藏滚动条(app){
            if(app){
                ...
                plus.webview.currentWebview().setStyle({scrollIndicator: "default"});
            }
        }
        
        function 取组件状态(name){
            var div = document.getElementById(name);
            var parent = div.parentNode;//获取组件的父元素
			if(parent.className!="mui-content" && parent != document.body){
				var displayStatus = window.getComputedStyle(parent).display;
			}else{
				var displayStatus = window.getComputedStyle(div).display;			
			}
        ...
        function 置窗口宽度(宽度){
            // 获取目标元素
            var element = document.querySelector('.mui-content');

            // 设置宽度和高度
            element.style.width = 宽度; 
            element.style.margin = "0 auto"; // 左右居中
        }
        ...
        function 置窗口高度(高度){
            ...
            element.style.overflow = "hidden"; // 超出部分隐藏
        }
        ...
        window["窗口操作"]["置窗口高度"]=置窗口高度;
        window["窗口操作"]["置窗口宽度"]=置窗口宽度;       
        ...
        window["窗口操作"]["设置状态栏字体颜色"]=设置状态栏字体颜色; 
        ...
        window["窗口操作"]["隐藏滚动条"]=隐藏滚动条;
        window["窗口操作"]["置组件距离顶部"]=置组件距离顶部;
        window["窗口操作"]["置组件距离顶部2"]=置组件距离顶部2;
        window["窗口操作"]["置组件自定义边框"]=置组件自定义边框;
        window["窗口操作"]["置组件自定义颜色"]=置组件自定义颜色;
    })();
